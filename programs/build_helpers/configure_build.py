...
class MyParser(argparse.ArgumentParser):
    ...
    def format_help(self):
        ...
        helplines.append(
            "  CMAKEOPTS \t\tadditional options to pass directly to cmake")
...
def parse_arguments(src_default):
    ...
    def try_default_dir_from_env(args, flag_name, metavar_name, env_name=None):
        ...
        if hasattr(args, flag_name):
            if not getattr(args, flag_name):
                parser.error(
                    "{} must be a valid directory".format(metavar_name))
        else:
            ...
            if directory:
                ...
                if not directory:
                    parser.error(
                        "{} environment variable must be a valid directory (or alternatively specify {} on command line)".format(
                            env_name, metavar_name))
                else:
                    setattr(args, flag_name, directory)

    parser = MyParser(
        description="Helper function to call cmake with appropriate configuration to build Project.")
    parser.add_argument("--sys-root", metavar="SYS_ROOT", type=convert_to_dir,
    ...
    parser.add_argument("--boost-dir", metavar="BOOST_ROOT",
    ...
    build_type.add_argument("-d", "--debug", dest="release",
                            action="store_false", default=argparse.SUPPRESS,
                            help="built with CMAKE_BUILD_TYPE=DEBUG")
    parser.add_argument("--win", "--windows", dest="windows",
                        action="store_true", default=argparse.SUPPRESS,
                        help="cross compile for Windows using MinGW")
    parser.add_argument("--src", dest="source_dir", metavar="SOURCEDIR",
                        type=convert_to_dir, default=argparse.SUPPRESS,
                        help="Project source directory (if omitted, will assume is at ../.. relative to location of this script)")
    parser.add_argument("additional_args", metavar="CMAKEOPTS",
                        nargs=argparse.REMAINDER, help=argparse.SUPPRESS)

    parser.set_defaults(low_mem_node=False, release=True, windows=False,
                        source_dir=src_default)
    ...
    try_default_dir_from_env(args, "openssl_dir", "OPENSSL_ROOT",
                             "OPENSSL_ROOT_DIR")
...
def find_boost_version(boost_dir):
    ...
    m = re.search('^#define BOOST_VERSION (?P<version>\d+)$',
                  boost_version_header_contents, flags=re.MULTILINE)
    try:
...
def main(args):
    ...
    if len(root_search_paths) > 0:
        command.append(
            "-DCMAKE_FIND_ROOT_PATH={}".format(";".join(root_search_paths)))
