...
Anyone (even you!) can edit their account to contain 1 million VIZ.  It's really
...
yourself 1 million VIZ.  Just like it's really impossible for your bank to physically stop
...
the fake VIZ or fake dollars you "gave" yourself).  So you can believe whatever you want
about your balance and rewrite the rules of your own bookkeeping system to show you whatever balance you
want to be shown, but as soon as you try to actually spend VIZ (or dollars) that you don't actually have,
...
(see `libraries/plugins/debug_node/include/graphene/plugins/debug_node/debug_node_api.hpp`
...
Okay, let's run `vizd`.  It should start immediately with no blocks.  We can ask it to read blocks from the directory we saved earlier:
...
Now that we've reset its key, we can take control of it in the wallet:
    ...
    list_my_accounts
    transfer steemit dantheman "1.234 VIZ" "make -j100 money" true
    list_my_accounts
